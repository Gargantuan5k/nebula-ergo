...
package spi_device_pkg;
  ...
  // Literal mirror of NumTotalCmdInfo (= NumCmdInfo(24) - 1 + 1(En4B) + 1(Ex4B) + 1(WrEn) +
  // 1(WrDi) + 1 = 28), for use anywhere NumTotalCmdInfo would size a port (struct-typed array
  // bounds like cmd_info_t [NumTotalCmdInfoW-1:0], or derived widths like CmdInfoIdxW below).
  // sv2v re-expands enum members back into their defining arithmetic wherever referenced and
  // wraps that in an unresolvable runtime cast function (sv2v_cast_32) when the result crosses
  // into a port-width context -- referencing NumTotalCmdInfo directly, even through another
  // named parameter, still triggers it. A literal has no arithmetic left to wrap, and $clog2 of
  // a plain literal (as opposed to $clog2 of an enum-derived expression) folds fine. MUST be
  // kept in sync with the cmd_info_index_e enum chain / NumCmdInfo if either changes -- there is
  // no compile-time check enforcing this given the above.
  parameter int unsigned NumTotalCmdInfoW = 28;

  parameter int unsigned CmdInfoIdxW = $clog2(NumTotalCmdInfoW);
  ...
  parameter logic [31:0] SramIngressByteOffset =
    32'(SPI_DEVICE_INGRESS_BUFFER_OFFSET - SPI_DEVICE_EGRESS_BUFFER_OFFSET);
