...
(
  ...
  input cmd_info_t [NumTotalCmdInfoW-1:0] cmd_info_i,
  ...
  // Pure lint-suppression (unused_cmdinfo_members feeds nothing else) -- excluded from
  // synthesis since sv2v's handling of this loop's bound consistently re-derives it from the
  // cmd_info_index_e enum's defining arithmetic (producing an unresolvable runtime cast
  // function for Yosys) regardless of what literal/parameter is written in the loop condition.
`ifndef SYNTHESIS
  logic unused_cmdinfo_members;
  always_comb begin
    unused_cmdinfo_members = 1'b 0;
    for (int unsigned i = 0 ; i < 28 ; i++) begin
