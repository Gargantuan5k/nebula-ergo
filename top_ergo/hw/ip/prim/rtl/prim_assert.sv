...
// NOTE(nebula): rewritten from the upstream `ifdef/`elsif/`elsif/`else chain
// into nested `ifdef/`else/`endif. sv2v's preprocessor only supports the
// classic `ifdef/`ifndef/`else/`endif directives, not `elsif — with the
// `elsif form, sv2v was observed falling through to the final `else branch
// (prim_assert_standard_macros.svh, full SVA incl. repetition operators
// like [*2]) even when SYNTHESIS was defined, causing a parse error deep
// inside modules that use those constructs (e.g. prim_alert_receiver.sv).
// Semantics are unchanged: VERILATOR > SYNTHESIS > YOSYS > default.
`ifdef VERILATOR
