...
#(
  ...
  input rst_main_div2_ni,
  input rst_main_div10_ni,
  input rst_usb_div4_ni,
  input rst_exp_div2_ni,
  input rst_exp_div8_ni,
  input rst_exp2_div2_ni,
  input rst_exp2_div6_ni,

  // Resets for derived clock generation, root clock gating and related status
  input rst_root_ni,
  input rst_root_main_ni,
  input rst_root_main_div2_ni,
  input rst_root_main_div10_ni,
  input rst_root_io_ni,
  input rst_root_io_div2_ni,
  input rst_root_io_div4_ni,
  input rst_root_usb_ni,
  input rst_root_usb_div4_ni,
  input rst_root_exp_ni,
  input rst_root_exp_div2_ni,
  input rst_root_exp_div8_ni,
  input rst_root_exp2_ni,
  input rst_root_exp2_div2_ni,
  input rst_root_exp2_div6_ni,
  ...
  ////////////////////////////////////////////////////
  mubi4_t exp_step_down_req;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(1),
    .StabilityCheck(1),
    .ResetValue(MuBi4False)
  ) u_exp_step_down_req_sync (
    .clk_i(clk_exp),
    .rst_ni(rst_exp_ni),
    .mubi_i(div_step_down_req_i),
    .mubi_o({exp_step_down_req})
  );

  mubi4_t exp2_step_down_req;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(1),
    .StabilityCheck(1),
    .ResetValue(MuBi4False)
  ) u_exp2_step_down_req_sync (
    .clk_i(clk_exp2),
    .rst_ni(rst_exp2_ni),
    .mubi_i(div_step_down_req_i),
    .mubi_o({exp2_step_down_req})
  );
  ...
  mubi4_t main_step_down_req;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(1),
    .StabilityCheck(1),
    .ResetValue(MuBi4False)
  ) u_main_step_down_req_sync (
    .clk_i(clk_main),
    .rst_ni(rst_main_ni),
    .mubi_i(div_step_down_req_i),
    .mubi_o({main_step_down_req})
  );

  mubi4_t usb_step_down_req;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(1),
    .StabilityCheck(1),
    .ResetValue(MuBi4False)
  ) u_usb_step_down_req_sync (
    .clk_i(clk_usb),
    .rst_ni(rst_usb_ni),
    .mubi_i(div_step_down_req_i),
    .mubi_o({usb_step_down_req})
  );
  ...
  logic [8:0] step_down_acks;

  logic clk_io_div2;
  logic clk_io_div4;
  logic clk_main_div2;
  logic clk_main_div10;
  logic clk_usb_div4;
  logic clk_exp_div2;
  logic clk_exp_div8;
  logic clk_exp2_div2;
  ...
  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] main_div2_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_main_div2_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({main_div2_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(2)
  ) u_no_scan_main_div2_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_main_i),
    .rst_ni(rst_root_main_ni),
    .step_down_req_i(mubi4_test_true_strict(main_step_down_req)),
    .step_down_ack_o(step_down_acks[2]),
    .test_en_i(mubi4_test_true_strict(main_div2_div_scanmode[0])),
    .clk_o(clk_main_div2)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] main_div10_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_main_div10_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({main_div10_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(10)
  ) u_no_scan_main_div10_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_main_i),
    .rst_ni(rst_root_main_ni),
    .step_down_req_i(mubi4_test_true_strict(main_step_down_req)),
    .step_down_ack_o(step_down_acks[3]),
    .test_en_i(mubi4_test_true_strict(main_div10_div_scanmode[0])),
    .clk_o(clk_main_div10)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] usb_div4_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_usb_div4_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({usb_div4_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(4)
  ) u_no_scan_usb_div4_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_usb_i),
    .rst_ni(rst_root_usb_ni),
    .step_down_req_i(mubi4_test_true_strict(usb_step_down_req)),
    .step_down_ack_o(step_down_acks[4]),
    .test_en_i(mubi4_test_true_strict(usb_div4_div_scanmode[0])),
    .clk_o(clk_usb_div4)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] exp_div2_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_exp_div2_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({exp_div2_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(2)
  ) u_no_scan_exp_div2_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_exp_i),
    .rst_ni(rst_root_exp_ni),
    .step_down_req_i(mubi4_test_true_strict(exp_step_down_req)),
    .step_down_ack_o(step_down_acks[5]),
    .test_en_i(mubi4_test_true_strict(exp_div2_div_scanmode[0])),
    .clk_o(clk_exp_div2)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] exp_div8_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_exp_div8_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({exp_div8_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(8)
  ) u_no_scan_exp_div8_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_exp_i),
    .rst_ni(rst_root_exp_ni),
    .step_down_req_i(mubi4_test_true_strict(exp_step_down_req)),
    .step_down_ack_o(step_down_acks[6]),
    .test_en_i(mubi4_test_true_strict(exp_div8_div_scanmode[0])),
    .clk_o(clk_exp_div8)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] exp2_div2_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_exp2_div2_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({exp2_div2_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(2)
  ) u_no_scan_exp2_div2_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_exp2_i),
    .rst_ni(rst_root_exp2_ni),
    .step_down_req_i(mubi4_test_true_strict(exp2_step_down_req)),
    .step_down_ack_o(step_down_acks[7]),
    .test_en_i(mubi4_test_true_strict(exp2_div2_div_scanmode[0])),
    .clk_o(clk_exp2_div2)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] exp2_div6_div_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_exp2_div6_div_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o({exp2_div6_div_scanmode})
  );

  prim_clock_div #(
    .Divisor(6)
  ) u_no_scan_exp2_div6_div (
    // We're using the pre-occ hookup (*_i) version for clock derivation.
    .clk_i(clk_exp2_i),
    .rst_ni(rst_root_exp2_ni),
    .step_down_req_i(mubi4_test_true_strict(exp2_step_down_req)),
    .step_down_ack_o(step_down_acks[8]),
    .test_en_i(mubi4_test_true_strict(exp2_div6_div_scanmode[0])),
    .clk_o(clk_exp2_div6)
  );
  ...
  prim_clock_buf u_clk_exp_powerup_buf (
    .clk_i(clk_exp),
    .clk_o(clocks_o.clk_exp_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.exp_powerup = MuBi4False;
  prim_clock_buf u_clk_main_div2_powerup_buf (
    .clk_i(clk_main_div2),
    .clk_o(clocks_o.clk_main_div2_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.main_div2_powerup = MuBi4False;
  prim_clock_buf u_clk_main_div10_powerup_buf (
    .clk_i(clk_main_div10),
    .clk_o(clocks_o.clk_main_div10_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.main_div10_powerup = MuBi4False;
  prim_clock_buf u_clk_usb_div4_powerup_buf (
    .clk_i(clk_usb_div4),
    .clk_o(clocks_o.clk_usb_div4_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.usb_div4_powerup = MuBi4False;
  prim_clock_buf u_clk_exp_div2_powerup_buf (
    .clk_i(clk_exp_div2),
    .clk_o(clocks_o.clk_exp_div2_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.exp_div2_powerup = MuBi4False;
  prim_clock_buf u_clk_exp_div8_powerup_buf (
    .clk_i(clk_exp_div8),
    .clk_o(clocks_o.clk_exp_div8_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.exp_div8_powerup = MuBi4False;
  prim_clock_buf u_clk_exp2_powerup_buf (
    .clk_i(clk_exp2),
    .clk_o(clocks_o.clk_exp2_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.exp2_powerup = MuBi4False;
  prim_clock_buf u_clk_exp2_div2_powerup_buf (
    .clk_i(clk_exp2_div2),
    .clk_o(clocks_o.clk_exp2_div2_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.exp2_div2_powerup = MuBi4False;
  prim_clock_buf u_clk_exp2_div6_powerup_buf (
    .clk_i(clk_exp2_div6),
    .clk_o(clocks_o.clk_exp2_div6_powerup)
  );

  // clock gated indication for alert handler: these clocks are never gated.
  assign cg_en_o.exp2_div6_powerup = MuBi4False;
  ...
  logic pwrmgr_main_div2_en;
  logic pwrmgr_main_div10_en;
  assign pwrmgr_main_en = pwr_i.main_ip_clk_en;
  assign pwrmgr_main_div2_en = pwr_i.main_ip_clk_en;
  assign pwrmgr_main_div10_en = pwr_i.main_ip_clk_en;
  // clk_io family
  logic pwrmgr_io_en;
  ...
  // clk_usb family
  logic pwrmgr_usb_en;
  logic pwrmgr_usb_div4_en;
  assign pwrmgr_usb_en = pwr_i.usb_ip_clk_en;
  assign pwrmgr_usb_div4_en = pwr_i.usb_ip_clk_en;
  // clk_exp family
  logic pwrmgr_exp_en;
  logic pwrmgr_exp_div2_en;
  logic pwrmgr_exp_div8_en;
  assign pwrmgr_exp_en = pwr_i.exp_ip_clk_en;
  assign pwrmgr_exp_div2_en = pwr_i.exp_ip_clk_en;
  assign pwrmgr_exp_div8_en = pwr_i.exp_ip_clk_en;
  // clk_exp2 family
  logic pwrmgr_exp2_en;
  logic pwrmgr_exp2_div2_en;
  logic pwrmgr_exp2_div6_en;
  assign pwrmgr_exp2_en = pwr_i.exp2_ip_clk_en;
  assign pwrmgr_exp2_div2_en = pwr_i.exp2_ip_clk_en;
  assign pwrmgr_exp2_div6_en = pwr_i.exp2_ip_clk_en;
  ...
  logic clk_main_div2_en;
  logic clk_main_div2_root;
  clkmgr_root_ctrl u_main_div2_root_ctrl (
    .clk_i(clk_main_div2),
    .rst_ni(rst_root_main_div2_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_main_div2_en),
    .en_o(clk_main_div2_en),
    .clk_o(clk_main_div2_root)
  );
  assign main_ens[1] = clk_main_div2_en;

  logic clk_main_div10_en;
  logic clk_main_div10_root;
  clkmgr_root_ctrl u_main_div10_root_ctrl (
    .clk_i(clk_main_div10),
    .rst_ni(rst_root_main_div10_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_main_div10_en),
    .en_o(clk_main_div10_en),
    .clk_o(clk_main_div10_root)
  );
  assign main_ens[2] = clk_main_div10_en;
  ...
  logic clk_usb_div4_en;
  logic clk_usb_div4_root;
  clkmgr_root_ctrl u_usb_div4_root_ctrl (
    .clk_i(clk_usb_div4),
    .rst_ni(rst_root_usb_div4_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_usb_div4_en),
    .en_o(clk_usb_div4_en),
    .clk_o(clk_usb_div4_root)
  );
  assign usb_ens[1] = clk_usb_div4_en;
  ...
  logic clk_exp_en;
  logic clk_exp_root;
  clkmgr_root_ctrl u_exp_root_ctrl (
    .clk_i(clk_exp),
    .rst_ni(rst_root_exp_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_exp_en),
    .en_o(clk_exp_en),
    .clk_o(clk_exp_root)
  );
  assign exp_ens[0] = clk_exp_en;

  logic clk_exp_div2_en;
  logic clk_exp_div2_root;
  clkmgr_root_ctrl u_exp_div2_root_ctrl (
    .clk_i(clk_exp_div2),
    .rst_ni(rst_root_exp_div2_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_exp_div2_en),
    .en_o(clk_exp_div2_en),
    .clk_o(clk_exp_div2_root)
  );
  assign exp_ens[1] = clk_exp_div2_en;

  logic clk_exp_div8_en;
  logic clk_exp_div8_root;
  clkmgr_root_ctrl u_exp_div8_root_ctrl (
    .clk_i(clk_exp_div8),
    .rst_ni(rst_root_exp_div8_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_exp_div8_en),
    .en_o(clk_exp_div8_en),
    .clk_o(clk_exp_div8_root)
  );
  assign exp_ens[2] = clk_exp_div8_en;
  ...
  logic clk_exp2_en;
  logic clk_exp2_root;
  clkmgr_root_ctrl u_exp2_root_ctrl (
    .clk_i(clk_exp2),
    .rst_ni(rst_root_exp2_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_exp2_en),
    .en_o(clk_exp2_en),
    .clk_o(clk_exp2_root)
  );
  assign exp2_ens[0] = clk_exp2_en;

  logic clk_exp2_div2_en;
  logic clk_exp2_div2_root;
  clkmgr_root_ctrl u_exp2_div2_root_ctrl (
    .clk_i(clk_exp2_div2),
    .rst_ni(rst_root_exp2_div2_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_exp2_div2_en),
    .en_o(clk_exp2_div2_en),
    .clk_o(clk_exp2_div2_root)
  );
  assign exp2_ens[1] = clk_exp2_div2_en;

  logic clk_exp2_div6_en;
  logic clk_exp2_div6_root;
  clkmgr_root_ctrl u_exp2_div6_root_ctrl (
    .clk_i(clk_exp2_div6),
    .rst_ni(rst_root_exp2_div6_ni),
    .scanmode_i,
    .async_en_i(pwrmgr_exp2_div6_en),
    .en_o(clk_exp2_div6_en),
    .clk_o(clk_exp2_div6_root)
  );
  assign exp2_ens[2] = clk_exp2_div6_en;
  ...
  logic clk_exp_peri_sw_en;
  logic clk_exp_div2_peri_sw_en;
  logic clk_exp_div8_peri_sw_en;
  logic clk_main_div2_peri_sw_en;
  logic clk_main_div10_peri_sw_en;
  logic clk_usb_div4_peri_sw_en;
  logic clk_main_peri_sw_en;
  logic clk_usb_peri_sw_en;
  logic clk_exp2_peri_sw_en;
  logic clk_exp2_div2_peri_sw_en;
  logic clk_exp2_div6_peri_sw_en;
  ...
  prim_flop_2sync #(
    .Width(1)
  ) u_clk_exp_peri_sw_en_sync (
    .clk_i(clk_exp),
    .rst_ni(rst_exp_ni),
    .d_i(reg2hw.clk_enables.clk_exp_peri_en.q),
    .q_o(clk_exp_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_exp_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_exp_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_exp_peri_scanmode)
  );

  logic clk_exp_peri_combined_en;
  assign clk_exp_peri_combined_en = clk_exp_peri_sw_en & clk_exp_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_exp_peri_cg (
    .clk_i(clk_exp),
    .en_i(clk_exp_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_exp_peri_scanmode[0])),
    .clk_o(clocks_o.clk_exp_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_exp_peri (
    .clk_i(clk_exp),
    .rst_ni(rst_exp_ni),
    .mubi_i(((clk_exp_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.exp_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_exp_div2_peri_sw_en_sync (
    .clk_i(clk_exp_div2),
    .rst_ni(rst_exp_div2_ni),
    .d_i(reg2hw.clk_enables.clk_exp_div2_peri_en.q),
    .q_o(clk_exp_div2_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_exp_div2_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_exp_div2_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_exp_div2_peri_scanmode)
  );

  logic clk_exp_div2_peri_combined_en;
  assign clk_exp_div2_peri_combined_en = clk_exp_div2_peri_sw_en & clk_exp_div2_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_exp_div2_peri_cg (
    .clk_i(clk_exp_div2),
    .en_i(clk_exp_div2_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_exp_div2_peri_scanmode[0])),
    .clk_o(clocks_o.clk_exp_div2_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_exp_div2_peri (
    .clk_i(clk_exp_div2),
    .rst_ni(rst_exp_div2_ni),
    .mubi_i(((clk_exp_div2_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.exp_div2_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_exp_div8_peri_sw_en_sync (
    .clk_i(clk_exp_div8),
    .rst_ni(rst_exp_div8_ni),
    .d_i(reg2hw.clk_enables.clk_exp_div8_peri_en.q),
    .q_o(clk_exp_div8_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_exp_div8_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_exp_div8_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_exp_div8_peri_scanmode)
  );

  logic clk_exp_div8_peri_combined_en;
  assign clk_exp_div8_peri_combined_en = clk_exp_div8_peri_sw_en & clk_exp_div8_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_exp_div8_peri_cg (
    .clk_i(clk_exp_div8),
    .en_i(clk_exp_div8_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_exp_div8_peri_scanmode[0])),
    .clk_o(clocks_o.clk_exp_div8_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_exp_div8_peri (
    .clk_i(clk_exp_div8),
    .rst_ni(rst_exp_div8_ni),
    .mubi_i(((clk_exp_div8_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.exp_div8_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_main_div2_peri_sw_en_sync (
    .clk_i(clk_main_div2),
    .rst_ni(rst_main_div2_ni),
    .d_i(reg2hw.clk_enables.clk_main_div2_peri_en.q),
    .q_o(clk_main_div2_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_main_div2_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_main_div2_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_main_div2_peri_scanmode)
  );

  logic clk_main_div2_peri_combined_en;
  assign clk_main_div2_peri_combined_en = clk_main_div2_peri_sw_en & clk_main_div2_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_main_div2_peri_cg (
    .clk_i(clk_main_div2),
    .en_i(clk_main_div2_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_main_div2_peri_scanmode[0])),
    .clk_o(clocks_o.clk_main_div2_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_main_div2_peri (
    .clk_i(clk_main_div2),
    .rst_ni(rst_main_div2_ni),
    .mubi_i(((clk_main_div2_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.main_div2_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_main_div10_peri_sw_en_sync (
    .clk_i(clk_main_div10),
    .rst_ni(rst_main_div10_ni),
    .d_i(reg2hw.clk_enables.clk_main_div10_peri_en.q),
    .q_o(clk_main_div10_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_main_div10_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_main_div10_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_main_div10_peri_scanmode)
  );

  logic clk_main_div10_peri_combined_en;
  assign clk_main_div10_peri_combined_en = clk_main_div10_peri_sw_en & clk_main_div10_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_main_div10_peri_cg (
    .clk_i(clk_main_div10),
    .en_i(clk_main_div10_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_main_div10_peri_scanmode[0])),
    .clk_o(clocks_o.clk_main_div10_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_main_div10_peri (
    .clk_i(clk_main_div10),
    .rst_ni(rst_main_div10_ni),
    .mubi_i(((clk_main_div10_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.main_div10_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_usb_div4_peri_sw_en_sync (
    .clk_i(clk_usb_div4),
    .rst_ni(rst_usb_div4_ni),
    .d_i(reg2hw.clk_enables.clk_usb_div4_peri_en.q),
    .q_o(clk_usb_div4_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_usb_div4_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_usb_div4_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_usb_div4_peri_scanmode)
  );

  logic clk_usb_div4_peri_combined_en;
  assign clk_usb_div4_peri_combined_en = clk_usb_div4_peri_sw_en & clk_usb_div4_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_usb_div4_peri_cg (
    .clk_i(clk_usb_div4),
    .en_i(clk_usb_div4_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_usb_div4_peri_scanmode[0])),
    .clk_o(clocks_o.clk_usb_div4_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_usb_div4_peri (
    .clk_i(clk_usb_div4),
    .rst_ni(rst_usb_div4_ni),
    .mubi_i(((clk_usb_div4_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.usb_div4_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_main_peri_sw_en_sync (
    .clk_i(clk_main),
    .rst_ni(rst_main_ni),
    .d_i(reg2hw.clk_enables.clk_main_peri_en.q),
    .q_o(clk_main_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_main_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_main_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_main_peri_scanmode)
  );

  logic clk_main_peri_combined_en;
  assign clk_main_peri_combined_en = clk_main_peri_sw_en & clk_main_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_main_peri_cg (
    .clk_i(clk_main),
    .en_i(clk_main_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_main_peri_scanmode[0])),
  ...
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_main_peri (
    .clk_i(clk_main),
    .rst_ni(rst_main_ni),
    .mubi_i(((clk_main_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.main_peri)
  );
  ...
  prim_flop_2sync #(
    .Width(1)
  ) u_clk_exp2_peri_sw_en_sync (
    .clk_i(clk_exp2),
    .rst_ni(rst_exp2_ni),
    .d_i(reg2hw.clk_enables.clk_exp2_peri_en.q),
    .q_o(clk_exp2_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_exp2_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_exp2_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_exp2_peri_scanmode)
  );

  logic clk_exp2_peri_combined_en;
  assign clk_exp2_peri_combined_en = clk_exp2_peri_sw_en & clk_exp2_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_exp2_peri_cg (
    .clk_i(clk_exp2),
    .en_i(clk_exp2_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_exp2_peri_scanmode[0])),
    .clk_o(clocks_o.clk_exp2_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_exp2_peri (
    .clk_i(clk_exp2),
    .rst_ni(rst_exp2_ni),
    .mubi_i(((clk_exp2_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.exp2_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_exp2_div2_peri_sw_en_sync (
    .clk_i(clk_exp2_div2),
    .rst_ni(rst_exp2_div2_ni),
    .d_i(reg2hw.clk_enables.clk_exp2_div2_peri_en.q),
    .q_o(clk_exp2_div2_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_exp2_div2_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_exp2_div2_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_exp2_div2_peri_scanmode)
  );

  logic clk_exp2_div2_peri_combined_en;
  assign clk_exp2_div2_peri_combined_en = clk_exp2_div2_peri_sw_en & clk_exp2_div2_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_exp2_div2_peri_cg (
    .clk_i(clk_exp2_div2),
    .en_i(clk_exp2_div2_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_exp2_div2_peri_scanmode[0])),
    .clk_o(clocks_o.clk_exp2_div2_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_exp2_div2_peri (
    .clk_i(clk_exp2_div2),
    .rst_ni(rst_exp2_div2_ni),
    .mubi_i(((clk_exp2_div2_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.exp2_div2_peri)
  );

  prim_flop_2sync #(
    .Width(1)
  ) u_clk_exp2_div6_peri_sw_en_sync (
    .clk_i(clk_exp2_div6),
    .rst_ni(rst_exp2_div6_ni),
    .d_i(reg2hw.clk_enables.clk_exp2_div6_peri_en.q),
    .q_o(clk_exp2_div6_peri_sw_en)
  );

  // Declared as size 1 packed array to avoid FPV warning.
  prim_mubi_pkg::mubi4_t [0:0] clk_exp2_div6_peri_scanmode;
  prim_mubi4_sync #(
    .NumCopies(1),
    .AsyncOn(0)
  ) u_clk_exp2_div6_peri_scanmode_sync  (
    .clk_i,
    .rst_ni,
    .mubi_i(scanmode_i),
    .mubi_o(clk_exp2_div6_peri_scanmode)
  );

  logic clk_exp2_div6_peri_combined_en;
  assign clk_exp2_div6_peri_combined_en = clk_exp2_div6_peri_sw_en & clk_exp2_div6_en;
  prim_clock_gating #(
    .FpgaBufGlobal(1'b1) // This clock spans across multiple clock regions.
  ) u_clk_exp2_div6_peri_cg (
    .clk_i(clk_exp2_div6),
    .en_i(clk_exp2_div6_peri_combined_en),
    .test_en_i(mubi4_test_true_strict(clk_exp2_div6_peri_scanmode[0])),
    .clk_o(clocks_o.clk_exp2_div6_peri)
  );

  // clock gated indication for alert handler
  prim_mubi4_sender #(
    .ResetValue(MuBi4True)
  ) u_prim_mubi4_sender_clk_exp2_div6_peri (
    .clk_i(clk_exp2_div6),
    .rst_ni(rst_exp2_div6_ni),
    .mubi_i(((clk_exp2_div6_peri_combined_en) ? MuBi4False : MuBi4True)),
    .mubi_o(cg_en_o.exp2_div6_peri)
  );
