...
module pwrmgr_fsm import pwrmgr_pkg::*; import pwrmgr_reg_pkg::*;(
  ...
  // exp is not part of pwrmgr's low-power sequencing (see nebula-cogito notes) --
  // always request it enabled so the clkmgr handshake resolves without gating it.
  assign ips_clk_en_o.exp_ip_clk_en = 1'b1;
  assign ips_clk_en_o.exp2_ip_clk_en = 1'b1;
  prim_flop #(
    .Width(1),
    .ResetValue(1'b0)
  ) u_usb_ip_clk_en (
